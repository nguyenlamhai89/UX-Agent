...
description: Converts insights.md, mapped-transcript.md, optional full transcript Markdown files, and journey-map.md into an interactive Ant Design-like HTML research report with saturation, transcript, full-transcript drawer, and journey views.
---
...
The orchestrator should trigger this skill after the `saturate-insights` skill has completed, in order to visualize the final research findings.
...
- **Input File(s)**:
  ...
  - `transcript-<interviewee>.md` (optional, auto-discovered) — Full interview transcript shown in the right-side drawer. Place it beside `mapped-transcript.md`; the legacy `transcript_<interviewee>.md` filename is also supported.
...
- Discover full transcript files beside `mapped-transcript.md`, match them to interviewee columns using Unicode-insensitive normalized names, and prefer the `transcript-<interviewee>.md` convention over the legacy underscore variant.
- Render a footer row in the Transcript table with one `Xem tất cả` button per interviewee. Open the matching full transcript in an accessible right-side drawer, support Escape-to-close and focus restoration, and disable the action when no corresponding full transcript exists.
...
sequenceDiagram
    ...
    Orchestrator->>Skill: Execute (insights_path, transcript_path)
    activate Skill
    Skill->>Skill: Parse Markdown and populate templates deterministically
    Skill->>Skill: Discover and embed full interview transcripts
...
| --- | --- | --- |
| `INVALID_INPUT` | Missing insights.md or mapped-transcript.md. | Return error prompting the user to run previous skills first. |
| `PARSING_ERROR` | Failed to parse Markdown structure. | Return error prompting the user to check markdown formatting. |
| `FULL_TRANSCRIPT_MISSING` | No matching `transcript-<interviewee>.md` exists beside `mapped-transcript.md`. | Continue generating the dashboard and render that interviewee's `Xem tất cả` action as disabled. |
