...
def discover_full_transcripts(transcript_path, interviewee_names):
    """Match each interviewee to an adjacent transcript-*.md source file.

    The requested dash convention is preferred. The legacy underscore variant
    remains supported for existing projects generated by older workflows.
    """
    transcript_dir = Path(transcript_path).resolve().parent
    candidates = []
    for pattern in ('transcript-*.md', 'transcript_*.md'):
        for candidate in sorted(transcript_dir.glob(pattern)):
            prefix_length = len('transcript-') if candidate.name.startswith('transcript-') else len('transcript_')
            suffix = candidate.stem[prefix_length:]
            candidates.append((normalize_interviewee_name(suffix), candidate))

    matched = {}
    claimed_paths = set()
    for interviewee in interviewee_names:
        normalized_name = normalize_interviewee_name(interviewee)
        for normalized_suffix, candidate in candidates:
            if candidate in claimed_paths:
                continue
            if normalized_name and normalized_name == normalized_suffix:
                matched[interviewee] = candidate
...
def build_full_transcript_ui(interviewee_names, transcript_path):
    """Build the table footer and embedded right-side transcript drawer."""
    transcript_files = discover_full_transcripts(transcript_path, interviewee_names)
    footer_cells = [
    ...
    for index, interviewee in enumerate(interviewee_names):
        ...
        transcript_file = transcript_files.get(interviewee)
        ...
        button_classes = (
            ...
            'inline-flex items-center justify-center rounded-lg border border-slate-200 bg-slate-100 '
            'px-3 py-2 text-sm font-semibold text-slate-400 cursor-not-allowed'
        )
        disabled_attributes = '' if has_transcript else ' disabled aria-disabled="true"'
        footer_cells.append(
            ...
            f'aria-controls="full-transcript-drawer" title="{button_title}"{disabled_attributes}>'
        ...
        if has_transcript:
            ...
            transcript_html = (
                ...
                'Không tìm thấy tệp <code>transcript-&lt;tên người tham gia&gt;.md</code> tương ứng.'
                '</div>'
            )
            source_label = 'Không có tệp nguồn'
...
async def main():
    ...
    transcript_headers = extract_transcript_headers(transcript_md, audio_names)
    transcript_footer, transcript_drawer = build_full_transcript_ui(transcript_headers, transcript_path)
