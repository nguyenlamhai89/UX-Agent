...
- Create a `<pdf-name>/` output folder before processing each discovered PDF.
- Write all per-PDF artifacts inside that folder.
- Always create `<pdf-name>/original.md` in the source language.
- Create `<pdf-name>/vie.md` only when the source paper is not already Vietnamese.
- Create `<pdf-name>/Asset/` and export every image/visual occurrence from the source PDF.
...
3. For local/file sources, each `<pdf-name>/` folder is created beside its source PDF.
4. For remote sources, each `<pdf-name>/` folder is created under:
...
- Custom Instructions for discovery, extraction, reconstruction, translation, validation, publication, and cleanup.
- Mermaid sequence diagram showing local/remote discovery, per-PDF staging, built-in-AI processing, validation, and batch results.
...
- Filename sanitization and collision-safe output placement.
- PDF signature, encryption, page, text-layer, layout, and visual inspection.
- Per-page rendering and asset extraction.
- Canonical page/block inventory generation.
- Built-in-AI task bundle generation and AI-response schema validation.
- Markdown rendering from validated canonical blocks.
- Vietnamese structural-parity validation.
- Transactional publication, skip/force behavior, batch status, and cleanup.

The Python module will not import an external LLM SDK. Instead, it will expose a file-based AI boundary:

1. Python prepares a deterministic source inventory and page-render bundle.
2. The skill's host agent uses built-in AI to create schema-constrained reconstruction/translation responses.
3. Python validates those responses, renders Markdown, and publishes only passing results.
...
prepare           Inspect one PDF and create its staged inventory/assets/renders.
...
Generate compact test PDFs inside pytest `tmp_path`; do not commit binary research papers. Fixtures will cover born-digital English, Vietnamese, scanned/image-only, mixed text/scans, multi-column layouts, tables, images, equations, footnotes, citations, bibliography, appendices, and colliding filenames.
...
For an English source named `paper-a.pdf`:
...
For a Vietnamese source named `paper-b.pdf`:
...
### 6.2 Create the required output folder before processing

1. Sanitize the filename stem using Unicode normalization, control-character removal, path-separator removal, length limits, and safe fallback names.
2. Resolve collisions deterministically. The first stem remains unchanged; later collisions receive a stable suffix derived from source order or a short source hash.
3. Create `<pdf-name>/` before inspection, rendering, AI reconstruction, or translation begins.
4. Create `<pdf-name>/.staging/<run-id>/` for all incomplete work.
5. Never write apparently valid `original.md`, `vie.md`, or `Asset/` until their publication transaction is ready.

### 6.3 Inspect the PDF deterministically

Use the bundled PDF stack where available:

- `pypdf` for metadata, encryption, page count, outline, and baseline text checks.
- `pdfplumber` for characters, words, text blocks, bounding boxes, tables, images, and layout evidence.
- Poppler tools such as `pdfinfo`, `pdftoppm`, `pdftocairo`, and `pdfimages` for independent inspection, rendering, and image extraction.
- Pillow for safe image normalization/cropping where required.

Record per page:

- Page number, size, rotation, and render path.
- Text-layer presence, character/word counts, and encoding warnings.
- Ordered text-block candidates and bounding boxes.
- Column candidates and reading-order ambiguity.
- Table regions and row/column evidence.
- Raster images, vector/drawing regions, charts, figures, captions, and logos.
- Equation candidates.
- Footnotes/endnotes.
- Header/footer repetition and whether it is meaningful content.
- Citation and reference candidates.
- Appendix and supplementary-section boundaries.
- Intentional blank-page evidence.

Classify each paper as born-digital, scanned, mixed, multi-column, or inaccessible. Text extraction alone will never be treated as proof of layout fidelity.

### 6.4 Extract and validate `Asset/`

For every image/visual occurrence:

1. Prefer the original embedded raster image at its available resolution.
2. Preserve a reliable vector visual as SVG when possible.
3. When direct extraction is missing, fragmented, or visually wrong, render and crop the source-page region at a controlled high resolution.
4. Name assets by page and occurrence, not by untrusted source strings.
5. Preserve repeated occurrences rather than silently deduplicating them; the manifest may note identical hashes.
6. Store source page, bounding box, type, hash, and caption association in the staging inventory.
7. Link meaningful figures/charts/logos at their correct position in Markdown. Keep non-content decorative images in `Asset/` and inventory them even if they are not inserted into prose.
...
### 6.5 Build a canonical content inventory

Create an internal staged representation containing stable IDs:
...
Every block records source page, bounding box/provenance, original text, ordering, and structural metadata. Tables record row/column counts, header cells, occupancy, and merged-cell spans. Figures record asset IDs and captions. Equations, citations, references, and footnotes retain stable identifiers.

The canonical inventory—not free-form AI prose—is the structural source of truth used to render both Markdown versions.

### 6.6 Use built-in AI for reconstruction
...
- Read scanned/image-only regions using vision/OCR capability.
- Reconstruct damaged or missing text-layer content from page renders.
- Classify headings, tables, figures, captions, equations, footnotes, references, and appendices.
- Confirm the source language.
- Fill schema-constrained canonical blocks.
...
- Preserve uncertainty as an explicit unresolved block.
- Return only the declared structured schema with original block IDs.
...
### 6.7 Render `original.md`

Render Markdown deterministically from the validated canonical inventory:
...
### 6.8 Detect Vietnamese and conditionally translate
...
### 6.9 Validate completeness and structural parity
...
- Reconcile section, block, table, figure, caption, equation, footnote, citation, reference, appendix, and meaningful metadata counts.
...
- Run a second pass against page renders and the final canonical inventory.
...
4. Replace `Asset/`, `original.md`, conditional `vie.md`, and `.conversion-manifest.json` using `os.replace`/directory renames where possible.
...
The pre-created `<pdf-name>/` directory may remain empty after a failed first conversion, but it must never contain a success manifest or apparently complete Markdown set.
...
| `PDF_PARSE_FAILED` | PDF is malformed or unreadable. | Try the independent parser once, then fail. |
...
1. Deterministic text/layout extraction.
2. Independent local parser/render verification.
3. Built-in-AI vision/OCR reconstruction from page or region renders.
4. Rendered visual fallback for figures/tables/equations while retaining recoverable text.
5. `review_required` when completeness remains uncertain.
...
- Keep all temporary files inside each output folder's `.staging/` directory.
- Never delete or alter source PDFs.
...
- Output folder creation before per-PDF processing.
- Born-digital, scanned, mixed, multi-column, encrypted, malformed, and blank-page inspection.
- Raster extraction, vector/rendered fallback, duplicate visual occurrences, captions, and missing assets.
- Canonical block schema validation.
...
6. Repeat execution:
   - Valid unchanged outputs return `skipped`.
...
2. Verify output placement, conditional `vie.md`, complete assets, UTF-8, headings, tables, equations, citations, references, and page coverage.
3. Re-run and verify `skipped`.
4. Re-run with `force: true` and verify transactional replacement.
5. Test a remote collection URL and confirm remote output placement and same-origin filtering.
...
3. Implement local/file/remote discovery, validation, security limits, hashing, filename sanitization, and collision handling.
4. Implement pre-created output folders, per-PDF staging, manifests, publication markers, rollback, skip/force, and cleanup.
5. Implement deterministic PDF inspection, page rendering, canonical inventories, and `Asset/` extraction/fallbacks.
6. Implement the schema-constrained built-in-AI handoff documented in `SKILL.md`.
7. Implement canonical Markdown rendering for original content.
8. Implement language confirmation, conditional translation, and invariant preservation.
9. Implement all completeness and structural-parity validators.
10. Implement structured batch results and stable error mapping.
11. Add unit and per-skill E2E tests.
12. Run dependency checks, validation, all skill tests, and the manual smoke test.
13. If any bug is found and fixed, document the bug/cause/resolution in `SKILL.md` and add a regression test.
