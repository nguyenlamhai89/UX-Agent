...
- **Resolved workspace**: Prefer `<folder_path>/Interview` when it contains both `full-questionnaire.md` and at least one `transcript_<audio_name>.md`; otherwise use `<folder_path>` directly. The questionnaire and all transcripts must be in the same resolved workspace.
- **Input File(s)**:
  - `<workspace>/full-questionnaire.md` — The structured questionnaire table generated by the `create-questionnaire-table` skill.
  - `<workspace>/transcript_<audio_name>.md` — Transcript files for the interviewees, including the Markdown emitted by `transcribe-audios`.
- **Examples**:
...
- **Location**: `file_path` — All output files are saved in the resolved workspace (`Interview/` for the normal workflow, otherwise the direct input folder).
- **Output File(s)**:
  - `<workspace>/mapped-transcript-<audio_name>.md` — Per-interviewee mapped transcript with questionnaire structure plus one interviewee column.
  - `<workspace>/mapped-transcript.md` — Combined mapped transcript with all interviewee columns side-by-side.
  - `<workspace>/mapped-transcript-review-<batch>.md` — Human review view with at most 5 interviewees.
  - `<workspace>/mapping-review-manifest.md` — Answered/N/A totals plus missing and overlapping timestamp signals.
  - `<workspace>/mapping-manifest.json` — Source signatures, attempts, task status, and output state.
  - `<workspace>/mapped-transcript.partial.md` — Optional explicit partial view; it never replaces the canonical output.
...
1. **Prepare before any skip decision**:
   ...
   ```
   This resolves the shared workspace first (complete `Interview/`, else direct folder), then compares SHA-256 source signatures, questionnaire changes, and the complete transcript filename set. It preserves stale mapped files as last-known-good outputs, chunks long transcripts, records orphans, and returns `canonical_current`.
...
6. **Mapping rules**:
   ...
   - When one cell needs multiple turns, append each complete turn as its own `[M+:SS] **<mark ...>...</mark>**` fragment in source order with `<br><br>` between fragments. Never merge text from different timestamps into one quote. Minutes may have more than three digits.
...
7. **Candidate format**: A candidate contains exactly one `# Mapped Transcript` heading, one five-column Markdown table, and one Mapping Summary. No other content or code fences are allowed.
   ...
   - Every non-`N/A` response includes the exact source timestamp and the complete source turn inside a yellow core-response highlight: `[M+:SS] **<mark style="background-color: yellow;">complete verbatim source turn</mark>**`.
   - Summary format is exact: `> **Mapping Summary**: Total rows: X | Answered: Y | N/A: Z | Transcript coverage: [M+:SS] to [M+:SS]`.
...
| `INVALID_MANIFEST` | `mapping-manifest.json` cannot be decoded or has an unsupported schema. | Preserve mapped outputs, remove or repair only the malformed manifest in the resolved workspace, then run `prepare` to rebuild controller state. |
| `NO_MANIFEST` | A stateful command was requested before `prepare`. | Run `mapping_pipeline.py prepare <folder_path>` and then request a new batch or status. |
| `UNKNOWN_TASK` | `record-success` or `record-failure` names no entry in the current manifest. | Do not write or promote a candidate; rerun `prepare`, call `next-batch`, and use the returned `audio_name` exactly. |
| `INVALID_STATE` | Completion was reported for a task that is not currently `running`. | Do not retry the same completion command; request the next batch or rerun `prepare` if sources changed. |
...
| `transcribe-audios` could write a direct-folder transcript while mapping required `Interview/` | The two skills had independent path assumptions, so an otherwise valid handoff could be invisible to the mapper. | Both skills now follow one workspace contract: use a complete `Interview/` handoff when present, otherwise use the direct folder; controller artifacts follow the same directory. |
| Long ElevenLabs recordings failed mapping timestamp validation | `transcribe-audios` can format minutes beyond 999, but mapping regexes capped the minute component at three digits. | All mapping timestamp parsers and summary checks now accept one or more minute digits, with a 1000:00 contract regression test. |
| Controller state errors lacked documented recovery | `INVALID_MANIFEST`, `NO_MANIFEST`, `UNKNOWN_TASK`, and `INVALID_STATE` were emitted by the CLI but omitted from operational guidance. | Added recovery instructions here and in the UX Interview orchestrator, with CLI regressions for every code. |
...
**Category 3: 🔗 Workflow Fit**
- [x] Add one shared input-directory resolution contract so `transcribe-audios` and `map-transcript` consistently use `Interview/` or the same explicit direct-folder mode; expand mapping timestamp patterns from a three-digit minute cap to one-or-more digits and add regression tests. *(I/O Contract Adherence: 7/10)*

**Category 4: 🛡️ Reliability & Error Handling**
- [x] Document every controller-emitted state code in this `SKILL.md` and `ux-interview/ORCHESTRATOR.md`, with explicit recovery actions for `INVALID_MANIFEST`, `NO_MANIFEST`, `UNKNOWN_TASK`, and `INVALID_STATE`; add CLI regression tests for each code. *(Error Rate: 7/10)*

**Category 5: 💰 Cost & Scalability**
- [x] Add a committed cross-skill contract test that generates `transcript_<audio_name>.md` with `transcribe-audios/format_transcription`, writes its metadata sidecar, then runs `map-transcript` prepare and candidate validation for `Interview/`, direct-folder behavior, multichannel speaker labels, and timestamps above 999 minutes. *(Unit Test Coverage & Pass Rate: 7/10)*
